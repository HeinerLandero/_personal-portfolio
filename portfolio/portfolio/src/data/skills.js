export const skills = [
    {
        'id':'skills01',
        'alt':'img skills',
        'name':'CSS3',
        'main': 0.9 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg',
        'categoria':'website',
        'content':'Clon de la página de Suzuki CR (Costa Rica) para la puesta en practica de las habilidades aprendidas',
        'proyects_relations':['_suzuki-clone','_library','LandingPageTaxisLibres','_labyrinth-game']
    },
    {
        'id':'skills02',
        'alt':'img skills',
        'name':'HTML5',
        'main': 0.9 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg',
        'categoria':'website',
        'content':'Página web para cliente en los Estados Unidos relacionado con servicios de metalmecánica',
        'proyects_relations':['_suzuki-clone','_library','LandingPageTaxisLibres','_labyrinth-game']
    },
    {
        'id':'skills03',
        'alt':'img skills',
        'name':'JavaScript',
        'main': 0.8 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg',
        'categoria':'website',
        'content':'skillso de participación para la empresa de transporte urbano en Bogotá Taxis Libres',
        'proyects_relations':['_suzuki-clone','_library','LandingPageTaxisLibres','_labyrinth-game']
    },
    {
        'id':'skills12',
        'alt':'img skills',
        'name':'Vue',
        'main': 0.8 ,
        'imagen':'https://cdn-icons-png.flaticon.com/512/15484/15484278.png',
        'categoria':'website',
        'content':'Página web para cliente en los Estados Unidos relacionado con servicios de metalmecánical',
        'proyects_relations':['personal-website','_library','LandingPageTaxisLibres','']
    },
    {
        'id':'skills05',
        'alt':'img skills',
        'name':'React.js',
        'main': 0.5 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg',
        'categoria':'website',
        'content':'Libreria de archivos desarrollada con el fin de almacenar documentación personal',
        'proyects_relations':['_portfolio','_library']
    },
    {
        'id':'skills04',
        'alt':'img skills',
        'name':'SASS',
        'main': 0.8 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sass/sass-original.svg',
        'categoria':'website',
        'content':'Landing page desarrollada con Bootstrap como posible opción de portafolio futuro',
        'proyects_relations':['_suzuki-clone','_library','LandingPageTaxisLibres']
    },
    {
        'id':'skills06',
        'alt':'img skills',
        'name':'Express',
        'main': 0.1 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/express/express-original.svg',
        'categoria':'website',
        'content':'Libreria de archivos desarrollada con el fin de almacenar documentación personal',
        'proyects_relations':['_API-REST-mdb ']
    },
    {
        'id':'skills07',
        'alt':'img skills',
        'name':'Node.js',
        'main': 0.15 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg',
        'categoria':'website',
        'content':'Libreria de archivos desarrollada con el fin de almacenar documentación personal',
        'proyects_relations':['_API-REST-mdb']
    },
    {
        'id':'skills08',
        'alt':'img skills',
        'name':'MongoDB',
        'main': 0.2 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg',
        'categoria':'website',
        'content':'Libreria de archivos desarrollada con el fin de almacenar documentación personal',
        'proyects_relations':['_API-REST-mdb']
    },
    {
        'id':'skills09',
        'alt':'img skills',
        'name':'Bootstrap',
        'main': 0.9 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg',
        'categoria':'website',
        'content':'Libreria de archivos desarrollada con el fin de almacenar documentación personal',
        'proyects_relations':['personal-website','_library','LandingPageTaxisLibres','']
    },
    {
        'id':'skills10',
        'alt':'img skills',
        'name':'Wordpress',
        'main': 0.7 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/wordpress/wordpress-original.svg',
        'categoria':'website',
        'content':'Página web para cliente en los Estados Unidos relacionado con servicios de metalmecánical',
        'proyects_relations':['personal-website','_library','LandingPageTaxisLibres']
    },
    {
        'id':'skills11',
        'alt':'img skills',
        'name':'JQuery',
        'main': 0.7,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jquery/jquery-original.svg',
        'categoria':'website',
        'content':'Página web para cliente en los Estados Unidos relacionado con servicios de metalmecánical',
        'proyects_relations':['personal-website','_library','LandingPageTaxisLibres']
    },
    {
        'id':'skills12',
        'alt':'img skills',
        'name':'PHP',
        'main': 0.8 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg',
        'categoria':'website',
        'content':'Página web para cliente en los Estados Unidos relacionado con servicios de metalmecánical',
        'proyects_relations':['personal-website','_library','LandingPageTaxisLibres','']
    },
    {
        'id':'skills12',
        'alt':'img skills',
        'name':'Angular',
        'main': 0.8 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/angular/angular-original.svg',
        'categoria':'website',
        'content':'Página web para cliente en los Estados Unidos relacionado con servicios de metalmecánical',
        'proyects_relations':['personal-website','_library','LandingPageTaxisLibres','']
    },
    {
        'id':'skills12',
        'alt':'img skills',
        'name':'Laravel',
        'main': 0.8 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/laravel/laravel-original.svg',
        'categoria':'website',
        'content':'Página web para cliente en los Estados Unidos relacionado con servicios de metalmecánical',
        'proyects_relations':['personal-website','_library','LandingPageTaxisLibres','']
    },
{
        'id':'skills12',
        'alt':'img skills',
        'name':'Java',
        'main': 0.8 ,
        'imagen':'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg',
        'categoria':'website',
        'content':'Página web para cliente en los Estados Unidos relacionado con servicios de metalmecánical',
        'proyects_relations':['personal-website','_library','LandingPageTaxisLibres','']
    }
    
];


